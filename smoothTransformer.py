...
def integralImage(defgrad):
    x = torch.cumsum(defgrad[:, 0, :, :], dim=2)
    y = torch.cumsum(defgrad[:, 1, :, :], dim=1)
    out = torch.stack([x,y], 1)
    return out - 1
...
def resample2D(im, sampling_grid, height, width, samples, channels):
    ...
    dim = width*height
    base = repeat(torch.arange(samples)*dim, dim)
    base = U.to_cuda(base)
    ...
    output = output.reshape(samples, height, width, channels)
    output = output.permute(0,3,1,2)

    return output

def smoothTransformer2D(im, deformable, c):
    defgrad = logisticGrowth(deformable, c)
